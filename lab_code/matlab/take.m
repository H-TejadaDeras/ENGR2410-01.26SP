...
if get(h, 'Value') == 1
    smu.set_source_function(1, 'VOLTAGE');
    smu.set_measure_function(1, 'CURRENT');
else
    smu.set_source_function(1, 'CURRENT');
    smu.set_measure_function(1, 'VOLTAGE');
end
h = findobj('Tag', [smu_name, '_ch2function']);
if get(h, 'Value') == 1
    smu.set_source_function(2, 'VOLTAGE');
    smu.set_measure_function(2, 'CURRENT');
else
    smu.set_source_function(2, 'CURRENT');
    smu.set_measure_function(2, 'VOLTAGE');
end
...
prim_fn = smu.get_source_function(prim_ch);
sec_fn = smu.get_source_function(sec_ch);

smu.set_source_value(prim_ch, prim_vals(1));
smu.set_source_value(sec_ch, sec_vals(1));
...
if length(y1lab)==0,
    ...
    if strcmp(smu.get_measure_function(1), 'CURRENT')
        y1lab = [y1lab, ' (A)'];
    else
        y1lab = [y1lab, ' (V)'];
    end
...
if length(y2lab)==0,
    ...
    if strcmp(smu.get_measure_function(2), 'CURRENT')
        y2lab = [y2lab, ' (A)'];
    else
        y2lab = [y2lab, ' (V)'];
    end;
...
buffer_lengths = [1, 1, 1; 1, 1, 1; 5, 3, 1; 10, 5, 3];
%settle_tol = [0.5, 1, 2, 100];
%wait_times = [0, 0, 0, 0, 0.01, 0.1];

for i = 1:length(sec_vals)
    smu.set_source_value(sec_ch, sec_vals(i));
    for j = 1:length(prim_vals)
        smu.set_source_value(prim_ch, prim_vals(j));

        if get(ch1autorange_h, 'Value') == 1
            smu.autorange_once(1);
        end
        if get(ch2autorange_h, 'Value') == 1
            smu.autorange_once(2);
        end
...
%        settling = get(settling_h, 'Value');
        ...
        if strcmp(y1lab, 'none') == 0
            buffer_length = buffer_lengths(smu.get_current_range(1) + 1, accuracy);
            if buffer_length == 1
                resp = smu.get_measure_value(1);
                ch1_meas(i, j) = resp{1};
            else
...
%            col = accuracy
            buffer_length = buffer_lengths(smu.get_current_range(2) + 1, accuracy);
            if buffer_length == 1
                resp = smu.get_measure_value(2);
                ch2_meas(i, j) = resp{1};
            else
                buffer = zeros(1, length(buffer_length));
                for k = 1:buffer_length
